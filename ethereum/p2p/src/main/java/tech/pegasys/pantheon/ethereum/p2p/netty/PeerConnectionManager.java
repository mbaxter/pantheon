...
class PeerConnectionManager implements DisconnectCallback {
  private static final Logger LOG = LogManager.getLogger();

  private final PeerConnectionRegistry activeConnections;
  private final Map<BytesValue, CompletableFuture<PeerConnection>> pendingConnections = new ConcurrentHashMap<>();
  ...
  public void setNodePermissioningController(NodePermissioningController nodePermissioningController, Blockchain blockchain) {
    checkNotNull(nodePermissioningController);
    checkNotNull(blockchain);
    final long blockAddedObserverId = blockchain.observeBlockAdded((e,b) -> disconnectInvalidPeers());
    shutdownCallbacks.add(() -> blockchain.removeObserver(blockAddedObserverId));
  }

  public void setEnodeUrl(EnodeURL ourEnodeURL) {
    this.ourEnodeUrl = ourEnodeURL;
  }

  public void setEnabled() {
    checkState(ourEnodeUrl != null, "Local enode must be set before " + getClass().getSimpleName() + " can be enabled.");
    enabled = true;
  }
  ...
  public boolean handleIncomingConnection(PeerConnection connection) {
    boolean connectionAccepted = true;
    BytesValue peerId = connection.getPeer().getNodeId();
    final CompletableFuture<PeerConnection> connectionFuture = CompletableFuture.completedFuture(connection);
    ...
    if (existingPendingConnection != null) {
      ...
      existingPendingConnection.whenComplete((peerConnection, err) -> {
        if (err != null) {
          return;
        }
  ...
  public CompletableFuture<PeerConnection> maybeConnect(final Peer peer) {
    ...
    if (existingPendingConnection != null) {
      ...
      ConnectionException exception = new DuplicatePeerConnectionException(
        "Attempt to connect to already connecting peer");
    ...
    if (countAllConnections() >= maxPeers) {
      ...
      connectionFuture.completeExceptionally(new DuplicatePeerConnectionException(
    ...
    if (!connectionFuture.isCompletedExceptionally()) {
      ...
      peerConnector.connect(peer)
        .whenComplete((peerConnection, err) -> {
          if (err != null) {
    ...
    return connectionFuture.whenComplete((peerConnection, err) -> {
      if (err != null) {
  ...
  public boolean removeMaintainedPeer(final Peer peer) {
    ...
    final Optional<PeerConnection> peerConnection = activeConnections.getConnectionForPeer(peer.getId());
  ...
  private boolean isPeerAllowed(final BytesValue peerId, final EnodeURL peerEnode) {
    ...
    return nodePermissioningController
      .map(c -> c.isPermitted(ourEnodeUrl, peerEnode))
      .orElse(true);
  }

  @Override
  public void onDisconnect(PeerConnection connection, DisconnectReason reason,
  ...
  private void shutdownPeerConnections() {
    activeConnections.getPeerConnections().forEach(p -> p.disconnect(DisconnectReason.CLIENT_QUITTING));
